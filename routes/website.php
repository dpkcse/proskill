...
if (! app()->runningInConsole()) {
    Auth::routes(['verify' => false]); // Disable built-in email verification
} else {
    Auth::routes(['verify' => false]);
}

// verify-email
Route::get('/verify-email/{token}', [\App\Http\Controllers\Auth\EmailVerificationController::class, 'verifyEmail'])->name('verification.verify.email');

Route::get('/verification/notice', [RegisterController::class, 'showVerificationNotice'])->name('verification.notice');

// Fallback routes for Laravel's built-in verification (to prevent errors)
Route::get('/email/verify', function () {
    return redirect()->route('login')->with('info', 'Please log in to access your account.');
})->name('verification.notice.fallback');

Route::get('/email/verify/{id?}/{hash?}', function ($id = null, $hash = null) {
    return redirect()->route('login')->with('info', 'Please use the verification link sent to your email.');
...
Route::post('/verification/resend/mail', [RegisterController::class, 'resendMail'])->name('verification.resend.mail');
Route::post('/verification/resend', [\App\Http\Controllers\Auth\EmailVerificationController::class, 'resendVerification'])->name('verification.resend');
