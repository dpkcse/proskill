...
$min_salary = $job->max_salary ? currencyConversion($job->max_salary, $currentCurrency->code) : 0;
$max_salary = $job->max_salary ? currencyConversion($job->max_salary, $currentCurrency->code) : 0;
@endphp
...
<script type="application/ld+json">
{
    ...
    "hiringOrganization" : {
        "@type" : "Organization",
        "name" : "{{ $job->company ? $job->company->user->name : config('app.name') }}",
        "sameAs" : "{{ config('app.url', 'https://www.google.com') }}",
        "logo" : "{{ $job->company ? $job->company->logo_url : '' }}"
    },
    "occupationalCategory": "{{ optional($job->profession)->name ?? '' }}",
                        ...
                        <div class="tw-flex tw-items-center tw-justify-between tw-gap-4 tw-flex-col md:tw-flex-row">
                                ...
                                <div class="tw-flex-grow">
                                    <div class="post-info2">
                                        ...
                                        <div class="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
                                            ...
                                            @if ($job->company)
                                            <a href="{{ route('website.employe.details', $job->company->user->username) }}"
                                                class="tw-mb-0 tw-whitespace-nowrap tw-text-lg tw-text-[#474C54]">
                                            <p class="tw-mb-0 tw-whitespace-nowrap tw-text-lg tw-text-[#474C54]">
                            ...
                            <div class="tw-flex !tw-items-start !tw-gap-2.5">
                                @if (moduleActive('CandidatePlan'))
                                    @if (!$hasActiveCandidatePlan || auth()->user()?->candidatePlan?->job_apply_limit > 0)
                                        @if ($job->can_apply)
                                            <div class="max-311">
                                                @if ($job->deadline_active)
                                                    @auth('user')
                                                        @if (auth()->user()->role == 'candidate')
                                                            @if (!$job->applied)
                                                                <button onclick="applyJobb({{ $job->id }}, '{{ $job->title }}')" class="btn btn-primary apply">
                                    ...
                                    @else
                                        @if (auth()->user()?->role == 'candidate')
                                            <button type="button" class="btn btn-primary btn-lg d-block">
                                                <span class="button-content-wrapper ">
                                                    <a href="{{ route('candidate.plan') }}" class="button-text tw-text-white hover:tw-text-white" style="text-decoration: none;"> {{ __('you_have_no_job_apply_limit') }}</a>
                                                </span>
                                            </button>
                                        @endif
                                    @endif
    ...
    <div class="container tw-pt-8">
        <div class="row">
            <div class="col-lg-7 rt-mb-lg-30">
                <div class="job-desc">
                    {!! preg_replace(
                        '/<oembed url="https:\/\/(?:www\.youtube\.com|youtu\.be)\/(?:watch\?v=)?([^?&]+).*"><\/oembed>/',
                ...
                <div class="p-32 border border-2 border-primary-50 rt-rounded-12 rt-mb-24 lg:max-536">
                    <div class="row">
                        ...
                        <div class="col-sm-6 salery tw-salery-border">
                            ...
                            <h2>{{ getFormatttedRangeSalary($job) }}</h2>
                            @else
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-4 rt-mb-32">
                                <div class="single-jSidebarWidget">
                                    ...
                                    <div class="iconbox-content">
                                        <div class="f-size-12 text-gray-500 uppercase text-uppercase rt-mb-6">
                                            {{ __('job_start_end_title') }}</div>
                                        <span class="d-block f-size-14 ft-wt-5 text-gray-900">
                                            {{ \Carbon\Carbon::parse($job->job_start)->format('d/m/Y') }} ⇒ {{ \Carbon\Carbon::parse($job->job_end)->format('d/m/Y') }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-4 rt-mb-32">
                                <div class="single-jSidebarWidget">
                                    <div class="icon-thumb">
                                        <i class="ph-briefcase f-size-30 text-primary-500"></i>
                                    </div>
                                    <div class="iconbox-content">
                                        <div class="f-size-12 text-gray-500 uppercase text-uppercase rt-mb-6">
                                            {{ __('profession') }}
                        ...
                        <div class="tw-flex tw-justify-between tw-items-center">
                            ...
                            <div class="tw-m-2">
                                <button onclick="resumeAddModal()" type="button"
                                    ...
                                    <span class="tw-mx-auto">{{ __('add_resume') }}</span>
                                </button>
                            </div>
                        </div>
