@if ((is_array($data) || $data instanceof \Illuminate\Database\Eloquent\Collection) )
                                    ...
                                    <div class="tw-w-[56px] tw-h-[56px]">
                                        @if ($job->company)
                                            <img class="tw-rounded-lg tw-w-[56px] tw-h-[56px]"
                                                src="{{ $job->company->logo_url ?? '' }}" alt="logo"
                                                draggable="false">
                                        @else
                    ...
                    <div class="tw-absolute tw-bottom-6 !tw-right-6">
                        <div class="text-primary-500 hoverbg-primary-50 plain-button icon-button">
                            ...
                            @if (auth()->user()->role == 'candidate')
                                <button class="tw-text-[#C8CCD1]" onclick="handleBookmarkClick()"
                                    type="button">
                                    @if ($job->bookmarked)
                ...
                <div class="tw-relative tw-h-full">
                    <a href="{{ $job->url ?? '#' }}" target="_blank" class="card tw-card extra-jobs jobcardStyle1">
                        <div class="tw-p-6">
                            ...
                            <div class="rt-single-icon-box tw-flex-wrap tw-items-end tw-gap-4">
                                <div class="iconbox-content">
                                    <div class="tw-mb-1 tw-inline-flex">
                                        <span class="tw-text-base tw-font-medium tw-text-[#18191C] tw-card-title">
    ...
    <button id="get-id-page" data-page="1"
        data-id="@if (isset($job->id)) {{ $job->id }} @else 0 @endif"
        class="newsButton btn btn-primary px-4 py-2 m-auto hidden">{{ __('Load id') }}</button>
