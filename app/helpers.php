    ...
    function socialMediaShareLinks(string $path, string $provider)
    {
        switch ($provider) {
            ...
            case 'twitter':
                ...
                break;
            case 'pinterest':
                $share_link = 'https://pinterest.com/pin/create/button/?url='.$path;
                break;
            case 'telegram':
...
if (! function_exists('currencyConversion')) {
    function currencyConversion($amount, $to = 'USD', $round = 2)
    {
        // GD: we optimise by caching the currency rate
        $to = strtoupper($to);
        $cacheKey = 'currency_rate_'.$to;
        $rate = Cache::remember($cacheKey, now()->addHours(6), function () use ($to) {
...
if (! function_exists('getFormattedNumber')) {
    function getFormattedNumber(
        $value,
        $currencyCode = 'USD',
        $locale = 'en_US', // Ignored in native version
        ...
        $thousandsSep = $groupingUsed ? ',' : '';
        $decimalSep = '.'; // no locale support in native PHP
        $formattedNumber = number_format($value, $precision, $decimalSep, $thousandsSep);
...
if (! function_exists('appCountryCode')) {
    function appCountryCode()
    {
        $setting = loadSetting();
...
if (! function_exists('getFormatttedRangeSalary')) {
    function getFormatttedRangeSalary($job)
    {
        // Defensive: check for required properties
        if (! isset($job->min_salary) || ! isset($job->max_salary)) {
            return '';
        }
        $min = currencyAmountShort($job->min_salary);
        $max = currencyAmountShort($job->max_salary);
        $type = isset($job->salary_type) && $job->salary_type ? $job->salary_type->name : '';
