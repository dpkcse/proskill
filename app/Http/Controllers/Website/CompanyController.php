...
class CompanyController extends Controller
{
    ...
    public function payPerJob()
    {
        try {
            ...
            $data['currency_symbol'] = config('templatecookie.currency_symbol');
    ...
    public function editJob(Job $job)
    {
        try {
            ...
            $data['salary_types'] = SalaryType::all();
            $data['tags'] = Tag::all()->sortBy('name');
            $data['start_day'] = date('yyyy-mm-dd', strtotime($job->created_at));
            $data['end_day'] = date('yyyy-mm-dd', strtotime($job->created_at.' +'.setting('job_deadline_expiration_limit').' days'));
            $data['skills'] = Skill::all()->sortBy('name');
            $data['professions'] = Profession::all()->sortBy('name');
    ...
    public function companyBookmarkCandidate(Request $request, Candidate $candidate)
    {
        try {
            ...
            if ($request->cat) {
                ...
                if (isset($user_plan) && $user_plan->candidate_cv_view_limit <= 0 && $user_plan->candidate_cv_view_limitation !== 'unlimited') {
                ...
                if (isset($user_plan) && $user_plan->candidate_cv_view_limitation !== 'unlimited') {
                    $user_plan->decrement('candidate_cv_view_limit');
                }
            }
