...
class WebsiteController extends Controller
{
    ...
    public function toggleBookmarkJob(Job $job)
    {
        try {
            ...
            // Update message handling based on whether the job was added or removed
            if ($check['attached'] == [1]) {
                $message = __('job_added_to_favorite_list');
                flashSuccess($message);
            } else {
                $message = __('job_removed_from_favorite_list');
                flashWarning($message); // Show flashError when job is removed
            }
    ...
    public function landingPageUpdate(Request $request)
    {
        try {
            ...
            $setting->landing_page = $request->current_theme;
